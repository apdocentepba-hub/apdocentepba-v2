...
function buildWhatsAppDigestMessage(user, alerts) {
  ...
  items.forEach((item, idx) => {
    ...
    const turno = norm10(item?.turno || "-") || "-";
    const nivel = norm10(item?.nivel || item?.nivel_modalidad || item?.modalidad || "-") || "-";
    ...
    const listadoMasAlto = norm10(item?.listado_origen_primero || "-") || "-";
...
async function handleTelegramWebhook(request, env) {
  ...
  if (!payload || !isUuid(payload)) {
    return json13({ ok: true, version: TELEGRAM_VERSION, ignored: true, reason: "start_payload_missing_or_invalid" });
  }
  const chatId = norm10(update?.message?.chat?.id);
  const chatType = norm10(update?.message?.chat?.type || "private");
  ...
  const next = await saveTelegramState(env, payload, {
    connected: true,
    chat_id: chatId,
    username: norm10(update?.message?.from?.username),
    first_name: norm10(update?.message?.from?.first_name),
  ...
  const confirmText = [
    ...
    "",
    "Ya pod\xE9s activar o pausar Telegram desde \u201CEditar preferencias reales\u201D en el panel.",
    "Cuando haya alertas nuevas compatibles, te van a llegar por ac\xE1."
  ].join("\n");
  ...
  await insertNotificationDeliveryLogs(env, baseLogRow({
    ...
    errorMessage: sent ? null : "No se pudo enviar confirmaci\xF3n de conexi\xF3n"
  }));
  return json13({
    ok: true,
    version: TELEGRAM_VERSION,
...
async function handleWhatsAppWebhook(request, env, ctx) {
  ...
  for (const message of messages) {
    ...
    if (!user?.id || user.activo === false) {
      const text = "No pude vincular este n\xFAmero con una cuenta activa de APDocentePBA. Revis\xE1 tu celular en el panel y volv\xE9 a escribir ALERTAS.";
    ...
    if (!message.text.includes("ALERTA")) {
      const text = "Escrib\xED ALERTAS para recibir tus alertas del momento por WhatsApp.";
    ...
    if (!entitlement.allowed) {
      const text = `Tu plan actual no tiene WhatsApp habilitado. Con ${entitlement.plan_name || "tu plan"} pod\xE9s usar email y Telegram.`;
    ...
    if (!prefs?.alertas_activas) {
      const text = "Tus alertas generales est\xE1n apagadas en preferencias. Activ\xE1 alertas en el panel y despu\xE9s escrib\xED ALERTAS otra vez.";
...
function buildTelegramDigestMessage(alerts) {
  ...
  return [
    `\u{1F514} APDocentePBA detect\xF3 ${alerts.length} alerta(s) nueva(s) compatible(s).`,
    "",
    ...visible.map(alertSummaryLine),
    hiddenCount ? `
+ ${hiddenCount} alerta(s) m\xE1s en tu panel.` : "",
    "",
    "Entr\xE1 al panel para ver el detalle completo y decidir r\xE1pido."
  ].filter(Boolean).join("\n");
}
