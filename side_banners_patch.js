...
function addStyles(){
  ...
  s.textContent=`
    .apd-side-banner{display:none;position:absolute;top:286px;z-index:5;--apd-side-w:min(320px,calc((100vw - 1180px)/2 - 34px));width:var(--apd-side-w);max-height:none;overflow:visible;border-radius:24px;padding:20px 18px;box-shadow:0 18px 44px rgba(16,36,61,.14)}
    .apd-side-banner *{box-sizing:border-box}
    .apd-side-banner-left{left:max(16px,calc((100vw - 1180px)/2 - var(--apd-side-w) - 18px));background:linear-gradient(180deg,#ffffff 0%,#f7fbff 100%);border:1px solid #dbe7f5;color:#061a37}
    .apd-side-banner-right{right:max(16px,calc((100vw - 1180px)/2 - var(--apd-side-w) - 18px));background:linear-gradient(135deg,#061a37,#0b2d5f 58%,#153d78 100%);border:1px solid rgba(255,255,255,.16);color:#fff}
    ...
    .apd-side-banner h4{margin:0 0 9px;font-size:23px;line-height:1.08;letter-spacing:-.03em}
    .apd-side-banner p{margin:0 0 14px;line-height:1.45;font-size:13px}
    .apd-side-product{border:1px solid #dbe7f5;border-radius:17px;padding:13px;background:#fff;box-shadow:0 8px 22px rgba(16,36,61,.05)}
    .apd-side-product+.apd-side-product{margin-top:11px}
    ...
    .apd-side-product h5{margin:0 0 9px;color:#0f3460;font-size:15px;line-height:1.22}.apd-side-price{display:block;margin:0 0 10px;color:#0f3460;font-size:24px;font-weight:900}
    .apd-side-btn{display:flex;justify-content:center;width:100%;padding:11px 12px;border-radius:13px;background:#2454f5;color:#fff!important;text-decoration:none;font-weight:900;font-size:12px}
    .apd-side-note{margin-top:11px;padding-top:11px;border-top:1px solid #dbe7f5;color:#5f718b;font-size:11px;line-height:1.4}
    .apd-news-item{display:block;text-decoration:none;border-radius:17px;padding:13px;background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.16);color:#fff!important}
    .apd-news-item+.apd-news-item{margin-top:10px}.apd-news-item small{display:block;margin-bottom:6px;color:#dbeafe;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.05em}.apd-news-item b{display:block;font-size:14px;line-height:1.32}
    .apd-news-actions{display:flex;flex-direction:column;gap:8px;margin-top:13px}.apd-news-actions a{display:flex;justify-content:center;border-radius:13px;padding:11px 12px;background:#fff;color:#0f3460!important;text-decoration:none;font-weight:900;font-size:12px}
    .apd-news-note{margin-top:11px;padding-top:11px;border-top:1px solid rgba(255,255,255,.14);color:#b9d7ff;font-size:11px;line-height:1.4}
    @media (min-width:1680px){body.apd-side-banners-inicio .apd-side-banner{display:block!important}body.apd-side-banners-inicio .apd-official-dgcye-banner,body.apd-side-banners-inicio .apd-secretaria-ad{display:none!important}}
    @media (max-width:1679px){.apd-side-banner{display:none!important}.apd-official-dgcye-banner,.apd-secretaria-ad{display:block!important}}
  `;
...
function getActiveTab(){
  var top=document.querySelector('.apd-top-tab.is-active[data-top-tab-key]');
  if(top) return top.dataset.topTabKey;
  var pane=document.querySelector('.panel-tab-pane.is-active');
  if(pane && pane.id) return pane.id.replace('panel-tab-pane-','');
  return localStorage.getItem('apd_panel_tab_v1') || 'inicio';
}

function updateVisibility(){
  var active=getActiveTab();
  document.body.classList.toggle('apd-side-banners-inicio', active==='inicio');
}
...
[300,600,1000,1800,3500].forEach(function(ms){setTimeout(install,ms);setTimeout(updateVisibility,ms+120);});
document.addEventListener('click',function(e){
  if(e.target.closest('[data-top-tab-key],[data-tab-key],[data-apd-go-tab]')) setTimeout(updateVisibility,80);
});
new MutationObserver(updateVisibility).observe(document.documentElement,{subtree:true,attributes:true,attributeFilter:['class']});
})();
