...
assert(/getSessionToken\(\)\s*\|\|\s*getUserToken\(\)/.test(patch), 'auth bearer does not prefer session token');
...
// The frontend may retain the user UUID for user_id parameters, but Authorization must prefer the session token.
const workerPatch = patch.match(/window\.workerFetchJson\s*=([\s\S]{0,1800})/)?.[1] || '';
assert(/getAuthBearer\(\)/.test(workerPatch), 'patched workerFetchJson does not use getAuthBearer');

console.log('PASS: frontend separates user id from bearer session for password and Google auth');
