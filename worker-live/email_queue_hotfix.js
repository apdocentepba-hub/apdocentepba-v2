...
function buildQueuedDigestHtml(user, alerts, appUrl) {
  const cards = (Array.isArray(alerts) ? alerts : []).map((alert) => {
    ...
    <div style="padding:14px 0;border-bottom:1px solid #e5e7eb;">
      ...
      <div style="font-size:11px;font-weight:800;color:#5d7290;letter-spacing:.08em;margin:10px 0 7px 0;">DATOS DE LA OFERTA</div>
      ...
      ${alert.curso_division ? `<div><b>CURSO / DIVISIÓN:</b> ${escapeHtml(alert.curso_division)}</div>` : ""}
      ${alert.modulos ? `<div><b>MÓDULOS:</b> ${escapeHtml(alert.modulos)}</div>` : ""}
      <div><b>DÍAS / HORA.PROB:</b> ${escapeHtml(alert.dias_horarios || "—")}</div>
      <div><b>VIGENCIA:</b> ${escapeHtml(vigencia)}</div>
      ...
      ${alert.estado ? `<div><b>ESTADO:</b> ${escapeHtml(alert.estado)}</div>` : ""}
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center">
      <table role="presentation" width="620" cellpadding="0" cellspacing="0" border="0" style="width:620px;max-width:620px;">
        <tr><td style="background:linear-gradient(135deg,#0f3460 0%,#1a4f8a 100%);color:#ffffff;padding:22px;border-radius:16px 16px 0 0;">
          ...
          <div style="font-size:13px;line-height:1.4;opacity:.9;margin-top:4px;">Resumen personalizado</div>
        </td></tr>
        <tr><td style="background:#ffffff;padding:18px;border:1px solid #dbe3f0;border-top:none;border-radius:0 0 16px 16px;">
          <div style="font-size:21px;font-weight:700;line-height:1.25;color:#0f3460;margin:0 0 10px 0;">Resumen APD compatible con tus preferencias</div>
          <div style="font-size:14px;line-height:1.5;color:#374151;margin:0 0 16px 0;">Hola ${escapeHtml(user?.nombre || "docente")}, encontramos coincidencias con las preferencias que cargaste en APDocentePBA. Te mostramos las más recientes para que puedas revisarlas.</div>
          ${cards}
