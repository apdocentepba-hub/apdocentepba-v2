...
function normalizeOfferPayload(offer) {
  return {
    ...
    offer_id: canonicalOfferIdentity(offer), source_offer_key: canonicalOfferIdentity(offer),
...
function renderMailOfferCard(row) {
  ...
  const motivoPid = String(p.pid_reason || "").trim() ||
    (p.pid_compatible ? "Compatible con tu PID" : "No compatible con tu PID");
                      ...
                      <tr>
                        ...
                        <td width="50%" style="padding:0 0 0 6px; vertical-align:top;">
                          ${mailMiniBox(
                            "LISTADO / AÑO PID",
                            escHtml(
                              `${p.pid_listado || "—"} · ${p.pid_anio || new Date().getFullYear()}`
                            )
                          )}
                        </td>
                      </tr>
...
async function handlePlanes(env) {
  ...
  const planes = (Array.isArray(rows) ? rows : [])
    .map((plan) => normalizePlanOut(plan))
    .map((plan) => applyFounderPricing(plan));
...
async function handleMercadoPagoCreateCheckoutLink(request, env) {
  ...
  if (!Number.isFinite(price) || price <= 0) {
    return json({
      ok: false,
      message: "El plan no tiene un precio válido para Mercado Pago.",
      plan_code: planCode,
  ...
  if (!checkoutConfigured) {
    return json({
      ...
      message: "Mercado Pago creó la preferencia, pero no devolvió checkout_url.",
...
function buildChannelAlertTextBlock(payload, options = {}) {
  ...
  if (showPid) {
    ...
    if (o.pid_bloque) lines.push(`🗂️ Bloque PID: ${o.pid_bloque}`);
...
function getFounderPriceInfo(code, fallbackPrice = null) {
  ...
  const original = Number.isFinite(originalFromPolicy) && originalFromPolicy > 0
    ? originalFromPolicy
    : (Number.isFinite(originalFromPlan) && originalFromPlan > promo ? originalFromPlan : promo);
...
function resolveCargoValue(input, cargoIndex) {
  ...
  // 1) PRIORIDAD ABSOLUTA: si el usuario escribió código explícito, resolver por código
  for (const code of explicitCodes) {
    ...
    if (byCodeMatches.length > 1) {
      // Elegir el más "base", no el más específico
  ...
  // 6) Fallback neutro si no resolvió nada
  return {
    kind: "cargo",
    code: "",
    canonical: n,
...
async function sendPendingEmailDigests(env, options = {}) {
  ...
  .sort((a, b) => {
    const tb =
      parseFechaFlexible(b?.last_seen_at)?.getTime() ||
      parseFechaFlexible(b?.updated_at)?.getTime() ||
      parseFechaFlexible(b?.created_at)?.getTime() ||
      0;

    const ta =
      parseFechaFlexible(a?.last_seen_at)?.getTime() ||
      parseFechaFlexible(a?.updated_at)?.getTime() ||
      parseFechaFlexible(a?.created_at)?.getTime() ||
      0;
...
function buildWhatsAppQueryDigestParts(alerts) {
  ...
  if (!Array.isArray(alerts) || alerts.length === 0) {
    return [
      "No encontré alertas activas para vos en este momento.\n\nPanel: https://alertasapd.com.ar"
    ];
  }
  ...
  for (let i = 0; i < alerts.length; i++) {
    ...
    const vigencia =
      pick(a, ["vigencia", "periodo_vigencia"]) ||
      [vigDesde, vigHasta].filter(Boolean).join(" / ") ||
      "-";
...
async function handleTelegramWebhook(request, env) {
  ...
  async function loadTelegramStoredAlerts(env, userId, limit = 10) {
    const rows = await supabaseSelect(
      env,
  ...
  async function loadTelegramLiveAlertsWithTimeout(env, userId, timeoutMs = 12000) {
    const timeout = new Promise((_, reject) => {
      setTimeout(() => reject(new Error("timeout_consultando_alertas")), timeoutMs);
    });

    const data = await Promise.race([
      construirAlertasParaUsuario(env, userId),
      timeout
    ]);
  ...
  if (!linkedUserId) {
    await sendTelegramText(
      env,
      chatId,
      "Todavía no vinculaste este chat con tu cuenta. Entrá al panel y usá el botón de conectar Telegram."
    ).catch(() => null);
  ...
  if (!user?.id) {
    await sendTelegramText(
      env,
      chatId,
      "No pude encontrar tu usuario vinculado. Volvé a conectar Telegram desde el panel."
    ).catch(() => null);
  ...
  if (user.activo === false) {
    await sendTelegramText(
      env,
      chatId,
      "Tu usuario figura inactivo. Revisá tu cuenta en el panel.\n\n🌐 https://alertasapd.com.ar"
    ).catch(() => null);
  ...
  if (command.kind === "alertas") {
    ...
    if (!telegramConnected) {
      await sendTelegramText(
        env,
        chatId,
        "Todavía no pude vincular este chat con tu cuenta. Entrá al panel, tocá conectar Telegram y después escribí ALERTAS."
      ).catch(() => null);
    ...
    if (!entitlement?.allowed) {
      await sendTelegramText(
        env,
        chatId,
        "Tu plan todavía no habilita Telegram.\n\n🌐 https://alertasapd.com.ar"
      ).catch(() => null);
    ...
    if (!alerts.length) {
      ...
      try {
        alerts = await loadTelegramLiveAlertsWithTimeout(env, user.id, 12000);
      } catch (err) {
        ...
        await sendTelegramText(
          env,
          chatId,
          "No pude consultar tus alertas ahora mismo. La búsqueda está tardando demasiado.\n\nProbá de nuevo en unos minutos o miralas en el panel:\n🌐 https://alertasapd.com.ar"
        ).catch(() => null);
    ...
    try {
      ...
      await sendTelegramText(
        env,
        chatId,
        `Encontré ${visibleAlerts.length} alerta(s) compatibles, pero no pude mandarte el detalle completo por Telegram.\n\nMiralas en el panel:\nhttps://alertasapd.com.ar\n\nEscribí ALERTAS otra vez si querés refrescar.`
      ).catch(() => null);
    }
...
async function handleWhatsAppWebhook(request, env) {
  ...
  async function waSend(env, to, text, context = "wa_send") {
    try {
  ...
  async function waLoadStoredAlerts(env, userId, limit = 50) {
    const rows = await supabaseSelect(
      env,
      `user_offer_state?user_id=eq.${encodeURIComponent(userId)}&is_active=eq.true&select=offer_id,offer_payload,last_seen_at&order=last_seen_at.desc&limit=${encodeURIComponent(String(limit))}`
    ).catch((err) => {
        ...
        for (const message of messages) {
          ...
          if (!user?.id) {
            await waSend(
              env,
              from,
              "No encontré una cuenta de APDocentePBA asociada a este número.\n\nGuardá tu celular en el panel y probá de nuevo:\nhttps://alertasapd.com.ar",
              "user_not_found"
            );
          ...
          if (!entitlement?.allowed) {
            await waSend(
              env,
              from,
              "WhatsApp queda reservado para el plan Insigne.\n\nEn tu plan actual seguís teniendo email y Telegram.",
              "plan_not_allowed"
            );
          ...
          if (!waIsAlertasCommand(inboundText)) {
            await waSend(
              env,
              from,
              "Hola. Escribí ALERTAS y te devuelvo las 5 ofertas compatibles más recientes.\n\nhttps://alertasapd.com.ar",
              "help"
            );
          ...
          const sortedAlerts = storedAlerts
            .slice()
            .sort((a, b) => waAlertTime(b) - waAlertTime(a));
          ...
          if (!visibleAlertsForRender.length) {
            await waSend(
              env,
              from,
              "No encontré alertas compatibles guardadas ahora.\n\nRevisá el panel:\nhttps://alertasapd.com.ar",
              "no_stored_alerts"
            );
...
async function debugLomasPreceptor(env, userId) {
  ...
  const candidatosPR = docs.filter((of) => {
    ...
    return (
      texto.includes("PRECEPTOR") ||
      texto.includes("PRECEPTORIA") ||
      texto.includes("(PR)") ||
      texto.includes("/PR")
    );
  });
