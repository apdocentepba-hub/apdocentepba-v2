(function(){
  ...
  if (window.__apdListboxFixLoaded) return;
  window.__apdListboxFixLoaded = true;
  ...
  function addStyles(){
    if(document.getElementById('apd-listbox-fix-style')) return;
    const style = document.createElement('style');
    style.id = 'apd-listbox-fix-style';
    style.textContent = `
  ...
  function renderList(input, list, items){
    list.innerHTML = ['<div class="ac-item" data-value="">NINGUNA</div>']
      .concat(items.map(item => `<div class="ac-item" data-value="${esc(item)}">${esc(item)}</div>`))
      .join('');
    list.dataset.open = '1';
  ...
  function bindField(inputId, listId, type){
    ...
    if(!input || !list || input.dataset.apdBoundListbox === '1') return;
    input.dataset.apdBoundListbox = '1';
    list.dataset.open = '0';
  ...
  function boot(){
    addStyles();
    DIST_INPUT_IDS.forEach((id, i) => bindField(id, DIST_LIST_IDS[i], 'district'));
    CARGO_INPUT_IDS.forEach((id, i) => bindField(id, CARGO_LIST_IDS[i], 'cargo'));
    const rec = document.getElementById('btn-recargar-panel');
    if(rec) rec.remove();
  }
