(function () {
  ...
  function buildAbcBookmarkletHref() {
    const source = `(async()=>{const TYPE='${ABC_IMPORT_TYPE}';const TARGET='*';const sleep=ms=>new Promise(r=>setTimeout(r,ms));const norm=s=>String(s||'').replace(/\\u00a0/g,' ').replace(/\\s+/g,' ').trim();const post=(status,payload,message)=>{if(window.opener&&!window.opener.closed){window.opener.postMessage({type:TYPE,status,payload,message,source:'abc-bookmarklet'},TARGET);}};function mapDocs(docs,dni){return(docs||[]).map(doc=>({anio:Number(doc.aniolistado||new Date().getFullYear()),tipo_listado:String(doc.tipo_listado||doc.listado||'OFICIAL').trim()||'OFICIAL',distrito:String(doc.distrito||'').trim(),cargo:String(doc.cargo_area||'').trim(),materia:String(doc.rama||'').trim(),puntaje:String(doc.puntaje||'').trim(),fuente:'abc_favorito',raw_text:JSON.stringify({source:'abc_favorito',dni: String(dni||''),documento:String(doc.documento||''),nombre:String(doc.nombre||''),apellido:String(doc.apellido||''),distrito:String(doc.distrito||''),rama:String(doc.rama||''),cargo_area:String(doc.cargo_area||''),puntaje:String(doc.puntaje||''),orden:doc.orden??null,aniolistado:String(doc.aniolistado||''),source_id:String(doc.id||''),source_timestamp:String(doc.timestamp||'')})})).filter(x=>x.cargo);}async function fetchByDni(dni){const bases=['/select/','/select','/listado-oficial/select/','/listado-oficial/select'];for(const base of bases){try{const all=[];let facets=null;for(let page=0;page<8;page++){const params=new URLSearchParams();params.set('q', 'busqueda='+dni);params.set('wt','json');params.set('rows','200');params.set('start',String(page*200));params.set('sort','orden asc');params.set('facet','true');params.set('facet.mincount','1');params.set('json.nl','map');params.append('facet.field','distrito');params.append('facet.field','rama');params.append('facet.field','cargo_area');params.append('facet.field','aniolistado');const res=await fetch(base+'?'+params.toString(),{credentials:'include',headers:{'Accept':'application/json, text/plain, */*','X-Requested-With':'XMLHttpRequest'}});const text=await res.text();const trimmed=String(text||'').trim();if(!trimmed||trimmed.startsWith('<!DOCTYPE')||trimmed.startsWith('<html'))throw new Error('html');const data=JSON.parse(trimmed);if(Number(data?.responseHeader?.status??1)!==0)throw new Error('json');const docs=Array.isArray(data?.response?.docs)?data.response.docs:[];if(!facets&&data?.facet_counts)facets=data.facet_counts;all.push(...docs);const found=Number(data?.response?.numFound||0);if(!docs.length||all.length>=found)return{docs:all,facets};}return{docs:all,facets};}catch(err){}}throw new Error('fetch-blocked');}function visible(el){return!!el&&el.offsetParent!==null;}function pageRangeText(){const all=[...document.querySelectorAll('body *')].filter(visible);const hit=all.find(el=>/Mostrando\\s+\\d+\\s+a\\s+\\d+\\s+de\\s+\\d+\\s+resultados/i.test(norm(el.innerText)));return hit?norm(hit.innerText):'';}function findCardCandidates(){const els=[...document.querySelectorAll('div, article, section, li')].filter(visible);const hits=els.filter(el=>{const t=norm(el.innerText);if(!t)return false;if(!/\\b\\d{7,9}\\b/.test(t))return false;if(!/Puntaje:/i.test(t))return false;if(!/Distrito:/i.test(t))return false;if(!/Cargo\\s*Area:/i.test(t))return false;if(t.length<80||t.length>1200)return false;return true;});return hits.filter(el=>!hits.some(other=>other!==el&&el.contains(other)));}function parseCardText(t){const text=String(t||'').replace(/\\u00a0/g,' ');const dni=/\\b(\\d{7,9})\\b/.exec(text)?.[1]||'';const puntaje=/Puntaje:\\s*([0-9.,]+)/i.exec(text)?.[1]||'';const orden=/Orden:\\s*([0-9]+)/i.exec(text)?.[1]||'';const cargo=(/Cargo\\s*Area:\\s*([\\s\\S]*?)(?=\\bApto\\s*F[ií]sico:|\\bDistrito:|\\bRama:|\\bRecalificaci[oó]n laboral:|\\bFecha:|$)/i.exec(text)?.[1]||'').replace(/\\s+/g,' ').trim();const distrito=(/Distrito:\\s*([\\s\\S]*?)(?=\\bRama:|\\bRecalificaci[oó]n laboral:|\\bFecha:|$)/i.exec(text)?.[1]||'').replace(/\\s+/g,' ').trim();const rama=(/Rama:\\s*([\\s\\S]*?)(?=\\bRecalificaci[oó]n laboral:|\\bFecha:|$)/i.exec(text)?.[1]||'').replace(/\\s+/g,' ').trim();return{dni:norm(dni),puntaje:norm(puntaje),orden:norm(orden),cargo:norm(cargo),distrito:norm(distrito),rama:norm(rama)};}function scrapeCurrentPage(){const cards=findCardCandidates();const items=[];for(const card of cards){const item=parseCardText(card.innerText);const key=[item.dni,item.cargo,item.puntaje,item.distrito,item.rama,item.orden].join('|');if(item.dni&&item.cargo)items.push({...item,key});}return items;}function nextButton(){const candidates=[...document.querySelectorAll('button,a,span,div')].filter(visible).filter(el=>norm(el.textContent)==='>');return candidates[candidates.length-1]||null;}async function scrapeRows(){const collected=new Map();for(let turn=0;turn<120;turn++){await sleep(1200);const rangeBefore=pageRangeText();const items=scrapeCurrentPage();for(const item of items)collected.set(item.key,item);const next=nextButton();if(!next)break;next.click();let changed=false;for(let i=0;i<14;i++){await sleep(500);const rangeAfter=pageRangeText();if(rangeAfter&&rangeAfter!==rangeBefore){changed=true;break;}}if(!changed)break;}return[...collected.values()].map(item=>({anio:new Date().getFullYear(),tipo_listado:'OFICIAL',distrito:item.distrito,cargo:item.cargo,materia:item.rama,puntaje:item.puntaje,fuente:'abc_favorito',raw_text:JSON.stringify({source:'abc_favorito',dni:item.dni,orden:item.orden,distrito:item.distrito,rama:item.rama,cargo_area:item.cargo,puntaje:item.puntaje})}));}try{const url=new URL(location.href);const dni=String(url.searchParams.get('apd_dni')||prompt('DNI para importar desde ABC:')||'').replace(/\\D/g,'');if(!dni)throw new Error('Necesitas indicar el DNI.');let payload=null;try{const fetched=await fetchByDni(dni);if(fetched?.docs?.length){payload={dni,rows:mapDocs(fetched.docs,dni),facets:fetched.facets||null,mode:'fetch-json'};}}catch(err){}if(!payload||!payload.rows?.length){const rows=await scrapeRows();if(!rows.length)throw new Error('No pude detectar resultados. Si la busqueda todavia no aparecio, hacela y volve a tocar el favorito.');payload={dni,rows,facets:null,mode:'scrape-dom'};}post('ok',payload,'Listados capturados desde ABC');setTimeout(()=>{try{window.close();}catch(e){}},250);}catch(err){post('error',null,String(err?.message||err));alert(String(err?.message||err));}})();`;
