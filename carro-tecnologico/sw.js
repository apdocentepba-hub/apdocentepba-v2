...
const BORRADOR_MAX_AGE_MS = 24 * 60 * 60 * 1000;
const restaurarBorradorBaseCarro = restaurarBorrador;

function descartarBorradorCarro_(motivo){
  try{localStorage.removeItem(DRAFT_KEY)}catch(e){}
  try{nuevoLote(false)}catch(e){}
  try{actualizarDraftBox(null)}catch(e){}
  if(motivo) mensaje(motivo,'warn');
}
...
restaurarBorrador = function(){
  try{
    ...
    if(!tieneContenidoBorrador(d)){
      descartarBorradorCarro_('');
      return;
    }

    const savedAt = Date.parse(d.savedAt || '');
    const vencido = !Number.isFinite(savedAt) || (Date.now() - savedAt) > BORRADOR_MAX_AGE_MS;
    if(vencido){
      descartarBorradorCarro_('El borrador anterior venció después de 24 horas y fue descartado.');
      return;
    }

    const cantidad = Array.isArray(d.codigos) ? d.codigos.length : 0;
    const tipo = d.tipoMovimiento === 'Devolución' ? 'devolución' : 'retiro';
    const fecha = new Date(savedAt).toLocaleString('es-AR');
    const box = $('draftBox');

    box.classList.remove('hide');
    box.innerHTML =
      '<div style="font-weight:800;font-size:16px;margin-bottom:4px">Hay un lote sin finalizar</div>' +
      '<div class="mini" style="margin-bottom:10px">' +
      (cantidad ? '<b>'+cantidad+'</b> netbook'+(cantidad===1?'':'s')+' · ' : '') +
      escapeHtml(tipo) + ' · guardado ' + escapeHtml(fecha) +
      '</div>' +
      '<div class="grid3">' +
      '<button id="continuarBorradorCarro" class="salida smallBtn">CONTINUAR</button>' +
      '<button id="descartarBorradorCarro" class="danger smallBtn">DESCARTAR</button>' +
      '</div>';

    $('continuarBorradorCarro').onclick = function(){
      restaurarBorradorBaseCarro();
      mensaje('Borrador retomado. Podés seguir donde lo dejaste.','ok');
    };

    $('descartarBorradorCarro').onclick = function(){
      descartarBorradorCarro_('Borrador descartado. Empezaste un lote nuevo.');
    };
  }catch(e){
    descartarBorradorCarro_('El borrador guardado no era válido y fue descartado.');
  }
};
