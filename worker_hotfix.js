...
const HOTFIX_VERSION = "2026-04-05-auth-sessions-1";
...
async function findUserByEmail(env, email) {
  const rows = await supabaseSelect(
    env,
    `users?email=ilike.${encodeURIComponent(email)}&select=id,nombre,apellido,email,celular,password_hash,google_sub,activo&limit=1`
  ).catch(() => []);
...
async function findUserByGoogleSub(env, sub) {
  const rows = await supabaseSelect(
    env,
    `users?google_sub=eq.${encodeURIComponent(sub)}&select=id,nombre,apellido,email,celular,password_hash,google_sub,activo&limit=1`
  ).catch(() => []);
  return Array.isArray(rows) ? rows[0] || null : null;
}
...
async function getUserById(env, userId) {
  const rows = await supabaseSelect(
    env,
    `users?id=eq.${encodeURIComponent(userId)}&select=id,nombre,apellido,email,celular,activo,es_admin,google_sub,password_hash,created_at,ultimo_login&limit=1`
  ).catch(() => []);
  return Array.isArray(rows) ? rows[0] || null : null;
}

async function touchUltimoLogin(env, userId) {
  await supabasePatchReturning(env, "users", `id=eq.${encodeURIComponent(userId)}`, {
...
async function ensureTrialIfNoSubscriptions(env, userId, email, source = "trial_auto_hotfix") {
  const existing = await supabaseSelect(
    env,
    `user_subscriptions?user_id=eq.${encodeURIComponent(userId)}&select=id&limit=1`
  ).catch(() => []);
  if (Array.isArray(existing) && existing.length > 0) return { ok: true, created: false };

  await supabaseInsertReturning(env, "user_subscriptions", {
    user_id: userId,
    plan_code: "TRIAL_7D",
    status: "ACTIVE",
    source,
...
async function createSession(env, userId, metodo = "password") {
  ...
  await supabaseInsertReturning(env, "sessions", {
    token,
    user_id: userId,
    metodo,
...
async function resolveAuthUser(env, request) {
  ...
  const sessions = await supabaseSelect(
    env,
    `sessions?token=eq.${encodeURIComponent(bearer)}&activo=eq.true&select=token,user_id,metodo,created_at,expires_at,activo&limit=1`
  ).catch(() => []);
  const session = Array.isArray(sessions) ? sessions[0] || null : null;

  if (session) {
    if (session.expires_at && new Date(session.expires_at).getTime() < Date.now()) {
      return { bearer, user: null, mode: "expired" };
    }
    const user = await getUserById(env, session.user_id);
    if (user?.activo !== false && user?.id) {
      return { bearer, user, mode: "session" };
    }
  }

  const legacyUser = await getUserById(env, bearer).catch(() => null);
  if (legacyUser?.activo !== false && legacyUser?.id) {
    return { bearer, user: legacyUser, mode: "legacy_user_id" };
  }
...
async function tryLegacyPasswordLogin(email, password) {
  ...
  for (const payload of payloads) {
    try {
      const res = await fetch(LEGACY_GAS_URL, {
        method: "POST",
...
async function ensureLocalUser(env, payload = {}) {
  ...
  return await supabaseInsertReturning(env, "users", {
    nombre: normalizeText(payload?.nombre || "Docente"),
    apellido: normalizeText(payload?.apellido || "-") || "-",
    email,
    celular: normalizeText(payload?.celular || ""),
    password_hash: secureHash,
    google_sub: payload?.google_sub ? String(payload.google_sub) : null,
    activo: true
  });
}
...
async function handleRegisterSecure(request, env) {
  ...
  const existing = await findUserByEmail(env, email);
  if (existing?.id) return json({ ok: false, error: "Ese email ya está registrado" }, 409);

  const passwordHash = await hashPasswordSecure(password);
  const nuevoUsuario = await supabaseInsertReturning(env, "users", {
    nombre,
    apellido,
    email,
    celular,
  ...
  if (!nuevoUsuario?.id) {
    return json({ ok: false, error: "No se pudo obtener el ID del usuario creado" }, 500);
  }
  ...
  return json({
    ...
    data: {
      id: nuevoUsuario.id,
      nombre: nuevoUsuario.nombre || nombre,
      apellido: nuevoUsuario.apellido || apellido,
      email: nuevoUsuario.email || email,
      celular: nuevoUsuario.celular || celular,
...
async function handleLoginSecure(request, env) {
  ...
  if (user?.id) {
    ...
    if (verified.ok) {
      if (verified.needsUpgrade) {
        const nextHash = await hashPasswordSecure(password);
        await supabasePatchReturning(env, "users", `id=eq.${encodeURIComponent(user.id)}`, { password_hash: nextHash }).catch(() => null);
      }
  ...
  if (legacy.ok) {
    ...
    user = await ensureLocalUser(env, {
      email,
      password,
      nombre: legacyUser?.nombre || legacyUser?.name || "",
      apellido: legacyUser?.apellido || legacyUser?.last_name || "",
      celular: legacyUser?.celular || legacyUser?.phone || ""
    });
...
async function handleGoogleAuthSecure(request, env) {
  ...
  if (user.activo === false) return json({ ok: false, message: "Usuario inactivo" }, 403);

  if (!user.google_sub || user.google_sub !== googleUser.sub) {
    await supabasePatchReturning(env, "users", `id=eq.${encodeURIComponent(user.id)}`, {
      google_sub: googleUser.sub,
      activo: true
    }).catch(() => null);
  }
  ...
  return json({
    ...
    user: {
      ...
      apellido: user.apellido || googleUser.apellido || "",
      email: user.email || googleUser.email || ""
    }
  });
}

const REWRITE_GET_PATHS = new Set([
  `${API_URL_PREFIX}/mi-plan`,
  `${API_URL_PREFIX}/mis-alertas`,
  `${API_URL_PREFIX}/historico-resumen`
]);

const REWRITE_POST_PATHS = new Set([
  `${API_URL_PREFIX}/guardar-preferencias`,
  `${API_URL_PREFIX}/capturar-historico-apd`,
  `${API_URL_PREFIX}/mercadopago/create-checkout-link`,
  `${API_URL_PREFIX}/whatsapp/test-send`
]);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === `${API_URL_PREFIX}/version` && request.method === "GET") {
      return json({ ok: true, version: HOTFIX_VERSION, worker_version: env.WORKER_URL || "worker-hotfix" });
    }

    if (path === `${API_URL_PREFIX}/register` && request.method === "POST") {
      try { return await handleRegisterSecure(request, env); } catch (err) { return json({ ok: false, message: err?.message || "No se pudo registrar" }, 500); }
    }

    if (path === `${API_URL_PREFIX}/login` && request.method === "POST") {
      try { return await handleLoginSecure(request, env); } catch (err) { return json({ ok: false, message: err?.message || "No se pudo iniciar sesión" }, 500); }
    }

    if (path === `${API_URL_PREFIX}/google-auth` && request.method === "POST") {
      try { return await handleGoogleAuthSecure(request, env); } catch (err) { return json({ ok: false, message: err?.message || "No se pudo ingresar con Google" }, 400); }
    }

    if (
      path.startsWith(`${API_URL_PREFIX}/profile/`) ||
      path.startsWith(`${API_URL_PREFIX}/listados/`) ||
      path.startsWith(`${API_URL_PREFIX}/eligibility/`)
    ) {
    ...
    if (REWRITE_GET_PATHS.has(path) || REWRITE_POST_PATHS.has(path)) {
      ...
      if (bearer) {
        ...
        if (!auth.user?.id) {
          return json({ ok: false, message: "No autenticado" }, 401);
        }
  ...
  async scheduled(controller, env, ctx) {
    if (typeof originalWorker?.scheduled === "function") {
      return await originalWorker.scheduled(controller, env, ctx);
    }
  }
};
