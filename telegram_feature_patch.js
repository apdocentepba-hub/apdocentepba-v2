(function () {
  ...
  function renderWhatsAppStatus(status, requested) {
    ...
    checkbox.disabled = !allowedByPlan;
    checkbox.checked = allowedByPlan ? isRequested : false;
    checkbox.style.pointerEvents = allowedByPlan ? 'auto' : 'none';
    checkbox.style.opacity = allowedByPlan ? '1' : '0.6';
    checkbox.style.filter = 'none';
    checkbox.closest('label')?.classList.toggle('is-disabled', !allowedByPlan);
