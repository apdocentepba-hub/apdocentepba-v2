...
export function isLikelySectionHeading(text, cells) {
  ...
  if (/^(TECNICO PROFESIONAL|SECUNDARIA|ADULTOS Y CFS|ARTISTICA|SUPERIOR|INICIAL|PRIMARIA|ESPECIAL|PSICOLOGIA|EDUCACION FISICA|FORMACION PROFESIONAL)\b/.test(norm)) return true;
  const compact = norm.replace(/[\s()\/.:-]/g, "");
  return !!compact && compact === compact.toUpperCase() && /[A-Z]/.test(compact) && !/\d/.test(compact);
}

export function isHeaderRow(cells, rowText) {
  const kinds = cells
    .map((cell) => detectFieldKind(cell.title || cell.text || ""))
    .filter(Boolean);
...
export function pickFirstPercentOrNumeric(texts, exceptValue = "") {
  for (const text of texts) {
    if (text === exceptValue) continue;
    if (isPercentLike(text)) return text;
  }
  for (const text of texts) {
    if (text === exceptValue) continue;
...
export function looksLikeNoiseDataRow(texts) {
  ...
  const longTextCount = meaningful.filter((text) => /[A-Za-zÁÉÍÓÚÑ]/.test(text) && cleanText(text).length >= 6).length;
  if (longTextCount > 0) return false;
...
export function finalizeSectionRow(candidate, context) {
  ...
  if (area && !looksLikeFileSize(area) && !detectFieldKind(area)) {
    context.lastArea = area;
  }
  ...
  return {
    bloque: cleanText(context.currentSection || ""),
    area,
    titulo,
    porcentaje,
    puntaje_total
...
export function parseRowHeuristically(cells, context) {
  ...
  if (looksLikeNoiseDataRow(texts)) return null;
  ...
  if (nonNumeric.length >= 2) {
    area = nonNumeric[0];
    titulo = nonNumeric.slice(1).join(" ");
  } else {
    area = context.lastArea || "";
    titulo = nonNumeric[0];
  }
...
export function deriveLegacyItems(sectionRows, fallbackItems) {
  ...
  for (const row of Array.isArray(sectionRows) ? sectionRows : []) {
    const item = {
      codigo: cleanText(row?.area || ""),
      rama: cleanText(row?.bloque || ""),
      puntaje: cleanText(row?.puntaje_total || row?.porcentaje || ""),
      fecha: ""
    };
